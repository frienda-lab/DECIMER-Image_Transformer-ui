...
def get_models(model_urls: dict):
    ...
    """
    # Prefer models bundled with the desktop application. In a PyInstaller
    # build, data files live under sys._MEIPASS; in development they live in
    # the repository's models directory.
    resource_root = Path(getattr(sys, "_MEIPASS", Path(HERE).parent))
    bundled_root = resource_root / "models" / "DECIMER-V2"
    bundled_paths = {
        "DECIMER": bundled_root / "DECIMER_model",
        "DECIMER_HandDrawn": bundled_root / "DECIMER_HandDrawn_model",
    }
    if all(path.exists() for path in bundled_paths.values()):
        model_paths = {name: str(path) for name, path in bundled_paths.items()}
    else:
        # Package/CLI fallback: download to the normal pystow data location.
