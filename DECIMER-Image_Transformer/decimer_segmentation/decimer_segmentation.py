...
def _load_model_internal() -> modellib.MaskRCNN:
    ...
    model_path = (
        os.path.dirname(bundled_model)
        if os.path.exists(bundled_model)
        else str(pystow.join("DECIMER-Segmentation_model"))
    )
...
# Keep model loading truly lazy for desktop applications. The upstream module
# loads weights during import, which blocks the UI before a job has started.
